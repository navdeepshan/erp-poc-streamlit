...
def page_consolidate():
    ...
    with st.expander("\U0001f5fa\ufe0f All POs Map — every PO ever created, not just this session"):
        st.caption("The Map tab after clicking Consolidate only shows what that specific run "
                  "just created. This shows everything in PO_Header right now, regardless of "
                  "when or how it was created — including Direct PO Entry. The map and the "
                  "line-item list below it are kept in sync — both filtered by the same date "
                  "range, so a PO isn't shown as a route while being invisible in the list.")
