      ...
      const clean = () => {
        try {
          ...
          if (!doc.getElementById('erp-reporting-chat-launcher')) {
            ...
            launcher.onclick = () => {
              let panel = doc.getElementById('erp-reporting-chat-panel');
              if (panel) {
                panel.style.display = panel.style.display === 'none' ? 'flex' : 'none';
                return;
              }
              panel = doc.createElement('section');
              panel.id = 'erp-reporting-chat-panel';
              panel.setAttribute('aria-label', 'ERP Reporting Assistant');
              panel.style.cssText = [
                'position:fixed','right:24px','bottom:88px','z-index:2147483646',
                'width:min(430px,calc(100vw - 32px))','height:min(680px,calc(100vh - 120px))',
                'display:flex','flex-direction:column','overflow:hidden',
                'background:#fff','border:1px solid #d9e2ec','border-radius:18px',
              ...
              header.style.cssText = [
                ...
                'background:#111827','color:#fff','font:600 15px system-ui'
              ].join(';');
              header.textContent = 'Reporting Assistant';

              const close = doc.createElement('button');
              close.textContent = '×';
              close.setAttribute('aria-label', 'Close ERP reporting chat');
              ...
              frame.title = 'ERP Reporting Chat';
              frame.src = window.top.location.origin + '/agent_console?chat_popup=1';
